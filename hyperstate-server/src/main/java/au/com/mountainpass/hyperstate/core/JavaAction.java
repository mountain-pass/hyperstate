...
public class JavaAction<T> extends Action<T> {
  ...
  public static HttpMethod determineMethodNature(final Method method) {
    ...
    if (ParameterizedType.class.isAssignableFrom(type.getClass())) {
      ...
      if (Class.class.isAssignableFrom(rawTypeClass)
          && CompletableFuture.class.isAssignableFrom((Class<?>) rawType)) {
        final Type[] typeParams = parameterizedType.getActualTypeArguments();
        if (typeParams.length == 1) {
          final Type typeParam = typeParams[0];
          if (Class.class.isAssignableFrom(typeParam.getClass())
              && Void.class.isAssignableFrom((Class<?>) typeParam)) {
            return HttpMethod.DELETE;
          } else if (Class.class.isAssignableFrom(typeParam.getClass())
              && CreatedEntity.class.isAssignableFrom((Class<?>) typeParam)) {
            return HttpMethod.POST;
          } else if (Class.class.isAssignableFrom(typeParam.getClass())
              && UpdatedEntity.class.isAssignableFrom((Class<?>) typeParam)) {
            return HttpMethod.PUT;
          } else if (ParameterizedType.class.isAssignableFrom(typeParam.getClass())
              && EntityWrapper.class
                  .isAssignableFrom((Class<?>) ((ParameterizedType) typeParam).getRawType())) {
  ...
  private static au.com.mountainpass.hyperstate.core.Parameter[] extractParameters(
    ...
    for (int i = 0; i < params.size(); ++i) {
      rval.add(new au.com.mountainpass.hyperstate.core.Parameter(params.get(i).getName()));
    }
    rval.add(new au.com.mountainpass.hyperstate.core.Parameter("action", PresentationType.SUBMIT,
        method.getName()));
    return rval.toArray(new au.com.mountainpass.hyperstate.core.Parameter[0]);
  }
  ...
  public JavaAction(final EntityWrapper<?> entity, final Method method) {
    super(null, method.getName(), new JavaLink(entity), extractParameters(method));
    this.method = method;
    this.entity = entity;
