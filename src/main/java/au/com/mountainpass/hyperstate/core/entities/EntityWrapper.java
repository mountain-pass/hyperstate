  ...
  protected EntityWrapper(String path, T properties, String label, String... natures) {
    ...
    for (Method method : methods) {
      ...
      if (httpMethod != null) {
        switch (httpMethod) {
        case DELETE:
          actions.put(method.getName(), new JavaAction<Void>(this, method));
          break;
        case POST:
          actions.put(method.getName(), new JavaAction<CreatedEntity>(this, method));
          break;
        case PUT:
          actions.put(method.getName(), new JavaAction<UpdatedEntity>(this, method));
        case GET:
          actions.put(method.getName(), new JavaAction<EntityWrapper<?>>(this, method));
        default:
        }
      }
    }
  ...
  public CompletableFuture<Collection<EntityRelationship>> getEntities(int page) {
    ...
    if (repository != null) {
      return repository.findChildren(this).thenApplyAsync(results -> {
        return results.skip(page * PAGE_SIZE).limit(PAGE_SIZE).collect(Collectors.toList());
      });
    }
